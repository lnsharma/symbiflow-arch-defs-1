...
def main():
    ...
    deviceData, io_cells, port_map = read_pinmapfile_data(args.pinmap_xml)
    ...
    with open(args.csv_file, mode='r') as csv_fp:
        ...
        for line in reader:
            ...
            for indx in range(len(port_name_list)):
                if port_name_list[indx] in port_map:
                    ...
                    print(
                        'Port name "{}" specified in csv file "{}" is invalid. Specify from port names in xml file "{}"'
                        .format(
                            line['port_name'], args.csv_file, args.pinmap_xml
                        ),
                        file=sys.stderr
                    )
                    sys.exit(1)
